# Copied from: https://github.com/python/cpython/blob/bfd0fbdc1352534b3c91b46dbae4285f5220acf4/Lib/test/support/os_helper.py
    ...
    if path is None:
        ...
        try:
            ...
            warnings.warn('tests may fail, unable to create '
                          'temporary directory {path!r}: {exc}'.format(path=path, exc=exc),
...
def change_cwd(path, quiet=False):
    ...
    try:
        ...
        warnings.warn('tests may fail, unable to change the current working '
                      'directory to {path!r}: {exc}'.format(path=path, exc=exc),
                      RuntimeWarning, stacklevel=3)
    try:
        yield os.getcwd()
    finally:
...
class FakePath:
    ...
    def __repr__(self):
        return '<FakePath {p!r}>'.format(p=self.path)
