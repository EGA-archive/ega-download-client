...
# Copied from https://github.com/python/cpython/blob/bfd0fbdc1352534b3c91b46dbae4285f5220acf4/Lib/test/support/__init__.py
...
def _requires_unix_version(sysname, min_version):
    ...
    return unittest.skipIf(
        skip,
        "{} version {} or higher required, not "
        "{}".format(sysname, min_version_txt, version_txt)
    )
...
def print_warning(msg):
    ...
    for line in msg.splitlines():
        print("Warning -- {}".format(line), file=sys.__stderr__, flush=True)
...
def reap_children():
    ...
    while True:
        ...
        print_warning("reap_children() reaped child process {}".format(pid))
...
def wait_process(pid, *, exitcode, timeout=None):
        ...
        while True:
            ...
            if dt > SHORT_TIMEOUT:
                ...
                raise AssertionError("process {} is still running "
                                     "after {dt:.1f} seconds".format(pid, dt=dt))
    ...
    if exitcode2 != exitcode:
        raise AssertionError("process {} exited with code {}, "
                             "but exit code {} is expected".format(pid, exitcode2, exitcode))

    # sanity check: it should not fail in practice
    if pid2 != pid:
        raise AssertionError("pid {} != pid {}".format(pid2, pid))
...
def skip_if_broken_multiprocessing_synchronize():
    ...
    if sys.platform == "linux":
        try:
            ...
            raise unittest.SkipTest("broken multiprocessing SemLock: {exc!r}".format(exc=exc))
