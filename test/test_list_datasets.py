...
def test_list_all_datasets(mock_data_server):
    mock_data_server.dataset_files = {'EGAD000000001': None, 'EGAD000000002': None, 'EGAD000000003': None}
    resp_json = pyega3.api_list_authorized_datasets(mock_data_server.token)
...
def test_error_when_token_is_not_valid(mock_data_server):
    with pytest.raises(requests.exceptions.HTTPError):
        pyega3.api_list_authorized_datasets(common.rand_str())


def test_exit_when_user_has_no_datasets(mock_data_server):
    mock_data_server.dataset_files = None

    with pytest.raises(SystemExit):
        pyega3.api_list_authorized_datasets(mock_data_server.token)
