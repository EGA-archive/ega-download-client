...
def test_calls_download_for_every_file_in_dataset(mocked_dfr, mock_data_server, dataset_with_files, mock_server_config,
    ...
    pyega3.download_dataset(mock_data_client, dataset_with_files.id, num_connections, None, None, 5,
                            5)

    assert len(dataset_with_files.files) == mocked_dfr.call_count
    mocked_dfr.assert_has_calls(
        [mock.call(mock_data_client, f['fileId'], num_connections, None, None, 5, 5, f['displayFileName'], f['fileName'], f['fileSize'],
                   f['unencryptedChecksum'], None) for f in
...
def test_no_error_if_md5_mismatch(mock_server_config, mock_data_server, dataset_with_files, mock_auth_client, mock_data_client):
