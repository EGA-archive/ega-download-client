...
def test_calls_download_for_every_file_in_dataset(mocked_dfr, mock_data_server, dataset_with_files, mock_server_config):
    with mock.patch("pyega3.pyega3.get_token", return_value=mock_data_server.token):
        ...
        pyega3.download_dataset(mock_server_config, creds, dataset_with_files.id, num_connections, None, None, 5, 5)
        ...
        mocked_dfr.assert_has_calls(
            [mock.call(creds, f['fileId'], f['displayFileName'], f['fileName'], f['fileSize'],
                       f['unencryptedChecksum'], num_connections, None, None, 5, 5, mock_server_config) for f in
...
def test_no_error_if_md5_mismatch(mock_server_config, mock_data_server, dataset_with_files):
    ...
    with mock.patch("pyega3.pyega3.get_token", return_value=mock_data_server.token):
        with mock.patch("pyega3.pyega3.download_file_retry", dfr_throws):
            pyega3.download_dataset(mock_server_config, {}, dataset_with_files.id, 1, None, None, 5, 5)
...
def test_download_unknown_dataset_does_not_call_download_file_retry(mock_server_config, mock_data_server):
    with mock.patch("pyega3.pyega3.download_file_retry") as mocked_dfr:
        with mock.patch("pyega3.pyega3.get_token", return_value=mock_data_server.token):
            ...
            pyega3.download_dataset(mock_server_config, {}, bad_dataset, 1, None, None, 5, 5)
            assert 0 == mocked_dfr.call_count


def test_download_legacy_dataset(mock_server_config):
    with pytest.raises(SystemExit):
        pyega3.download_dataset(mock_server_config, "credentials", "EGAD00000000003", "1", "output_dir",
                                "genomic_range_args")
